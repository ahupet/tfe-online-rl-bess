...
* **`main.py`**: The synchronized training pipeline for the benchmark off-policy deterministic (`TD3`) agent.
* **`ppo_main.py`**: The synchronized training pipeline for the benchmark on-policy stochastic (`PPO-clip`) agent, including checkpoints and early stopping metrics.
* **`evaluation.py`**: Automated validation execution pass for the `TD3` baseline model over target historical timelines.
* **`ppo_evaluation.py`**: Automated validation evaluation pass tracking recurrent `policy_state` arrays for the `PPO` network.
