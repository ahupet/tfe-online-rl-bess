# -*- coding: utf-8 -*-
"""
Long-Term Evaluation Script - TD3 Agent (V5 Logic) - 2020-2023
- Performance assessment on the long-term testing dataset tracks (Empirical Price Ladder 6h V3).
- Fixed Baseline Baseline Configuration (Neural weights frozen at the end of 2019).
- Generates a multi-sheet spreadsheet tracking Daily, Monthly, and Annual financial returns.
- Embedded Anti-Crash Python memory garbage collection layers.
...
# =============================================================================
DOSSIER_RUN = r"C:\Users\pcstudentgele11\TFE_HUPET_ACHILLE\TD3_without_LA_without_stairs\Run_TD3_20260418_123006_EARLY_STOP_WDW8_S3"
DOSSIER_AGENT = os.path.join(DOSSIER_RUN, "saved_policy_final")

# Establish output result logging target folders
SAVE_DIR = "Resultats_Eval_TD3_2020_2023_SEED3_FINAL"
os.makedirs(SAVE_DIR, exist_ok=True)
print(f"Dossier de sauvegarde des resultats cree : {SAVE_DIR}")
...
print(f"Loading TD3 agent's brain from: {DOSSIER_AGENT}...")
...
for date_str in jours_a_simuler:
    ...
    # ALIGNMENT CORRECTION: Calibrate processing steps boundaries onto 96-quarter day blocks
    ...
    for _ in range(nb_quarters_per_episode):
        ...
        # FIX: Append 'fraction_power' (true clipped action output) and apply layout color codes
    ...
    # --- INTRA-DAY PROFILE PLOTTING (CRIMSON RED GRAPHIC THEME SPECIFIC TO TD3) ---
    ...
    ax_price.set_title(f"Market Price Impact - {date_str} (TD3 V5)")
    ...
    ax_action_si.plot(x_axis, nrv_modified_list, color='crimson', label='Modified NRV', linewidth=1.5, alpha=0.8)
    ax_action_si.axhline(0, color='black', linewidth=0.8)
    ax_action_si.set_ylabel("NRV (MW)", color='crimson'); ax_action_si.tick_params(axis='y', labelcolor='crimson')
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(SAVE_DIR, f"Strategy_Day_{date_str}_TD3.png"), dpi=300)
    plt.close(fig) 
...
for ep, start_idx in enumerate(index_debuts_jours):
    ...
    if (ep + 1) % 100 == 0:
        ...
        # --- ANTI-CRASH MEMORY SAFEGUARD N°2: Explicit execution path RAM flush ---
        gc.collect()
...
excel_path = os.path.join(SAVE_DIR, "Gains_TD3_2020_2023.xlsx")
...
ax_f.plot(range(num_episodes), cumul_profit_export, color='crimson', linewidth=2)
ax_f.fill_between(range(num_episodes), cumul_profit_export, color='lightcoral', alpha=0.3)
ax_f.set_title("Cumulative Financial Performance TD3 - Fige (2020-2023)", fontsize=14, fontweight='bold')
ax_f.set_xlabel("Days Evaluated")
ax_f.set_ylabel("Total Profit (€)")
ax_f.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()

plot_global_path = os.path.join(SAVE_DIR, "Global_Results_2020_2023_TD3.png")
fig_f.savefig(plot_global_path, dpi=300)
plt.close('all')

print("\n" + "="*75)
print("              TD3 ANNUAL SUMMARY TABLE FOR LATEX")
print("="*75)
